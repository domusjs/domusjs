import { context, trace, SpanKind, SpanStatusCode, Tracer, Span } from '@opentelemetry/api';
...
export function Traceable(tracer: Tracer, options: TraceableOptions = {}): MethodDecorator {
  return function (target, propertyKey, descriptor: PropertyDescriptor): PropertyDescriptor {
    ...
    descriptor.value = function (...args: any[]) {
      const spanName = options.spanName ?? `${target.constructor.name}.${String(propertyKey)}`;
      ...
      if (options.logEvent) {
        const event =
          typeof options.logEvent === 'function' ? options.logEvent(args) : options.logEvent;
        span.addEvent(event);
      }
